...
A bottom border can be added with the `bottom-border` class.
...
Links are underlined by default. Remove underlines only if the context on the page indicates that the
text is a link even without the underline. For example, links in headings or a table of contents may
not need underlines if users can understand that they're links.
