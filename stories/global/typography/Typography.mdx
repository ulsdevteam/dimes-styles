...
import * as textStyleStories from './TextStyle.stories.js';
...
For a bolder link, use the `font--bold` class.

<Story of={linkStories.links} />

## Text color and font weight

### When to use
In the case where the default text color results in accessibility issues because of color
contrast with the background color or aesthetically don't look good, use text color classes
to change the element color.

Use `font--bold` to change any text weight to 600 bold.

### Best practices
Components like buttons, a header, or a table of contents already have various color options 
with different text colors, so use the specific component color classes when appropriate.
...
- For bold text use `text--bold`
- For white text use `text--white`
- For blue text use `text--blue`
- For orange text use `text--orange`

<Story of={textStyleStories.textStyle} />
